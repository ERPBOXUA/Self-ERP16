...
class TestMutualSettlements(AccountContractTestCommon):
    ...
    def test_reconcile_with_order(self):
        ...
        self.assertEqual(order1.amount_paid, 0)
        self.assertEqual(order1.amount_outstanding_payment, order1.amount_total)
        ...
        move_line1 = bank_move1.line_ids.filtered(lambda r: r.account_type == 'asset_receivable')
        self.assertEqual(len(move_line1), 1)
        self.assertEqual(move_line1.linked_sale_order_id, order1)
        ...
        self.assertEqual(float_compare(to_reconcile.get('content')[0].get('amount'), move_line1.credit, precision_digits=self.currency.decimal_places), 0)

        # check sale order payments
        self.assertEqual(order1.amount_paid, 10.5)
        self.assertEqual(order1.amount_outstanding_payment, order1.amount_total - 10.5)
        ...
        # check sale order payments
        self.assertEqual(order2.amount_paid, 0)
        self.assertEqual(order2.amount_outstanding_payment, order2.amount_total)
        ...
        # check sale order in a move line
        move_line2 = bank_move2.line_ids.filtered(lambda r: r.account_type == 'asset_receivable')
        self.assertEqual(len(move_line2), 1)
        self.assertEqual(move_line2.linked_sale_order_id, order1)

        # check order move lines
        self.assertNotEqual(move_line2, invoice_move_line)
        self.assertFalse(not move_line2.matched_debit_ids)
        self.assertEqual(len(move_line2.matched_debit_ids), 1)
        self.assertEqual(move_line2.matched_debit_ids.debit_move_id, invoice_move_line)
        ...
        self.assertEqual(float_compare(reconciled.get('content')[0].get('amount'), move_line2.credit, precision_digits=self.currency.decimal_places), 0)
        to_reconcile = invoice.invoice_outstanding_credits_debits_widget
        self.assertFalse(not to_reconcile)
        self.assertFalse(not to_reconcile.get('content'))
        self.assertEqual(len(to_reconcile.get('content')), 1)
        ...
        # check sale order payments
        self.assertEqual(order1.amount_paid, 10.5 + 22.44)
        self.assertEqual(order1.amount_outstanding_payment, order1.amount_total - 10.5 - 22.44)
        self.assertEqual(order2.amount_paid, 0)
        self.assertEqual(order2.amount_outstanding_payment, order2.amount_total)
        ...
        statement_line3 = self.create_contract_bank_statement_line(
            partner=self.partner_a,
            amount=invoice_move_line.amount_currency + 1000,
            sale_order=order2,
        )
        ...
        # check sale order in a move line
        move_line3 = bank_move3.line_ids.filtered(lambda r: r.account_type == 'asset_receivable')
        self.assertEqual(len(move_line3), 2)
        self.assertEqual(move_line3[0].linked_sale_order_id, order1)
        self.assertEqual(move_line3[1].linked_sale_order_id, order2)

        # check order move lines
        self.assertNotEqual(move_line3[0], invoice_move_line)
        self.assertNotEqual(move_line3[1], invoice_move_line)
        self.assertFalse(not move_line3[0].matched_debit_ids)
        self.assertEqual(len(move_line3[0].matched_debit_ids), 1)
        self.assertEqual(move_line3[0].matched_debit_ids.debit_move_id, invoice_move_line)
        ...
        self.assertEqual(float_compare(reconciled.get('content')[1].get('amount'), move_line3[0].credit, precision_digits=self.currency.decimal_places), 0)
        to_reconcile = invoice.invoice_outstanding_credits_debits_widget
        self.assertTrue(not to_reconcile)

        # @TODO: check sale order payments
        # self.assertEqual(order1.amount_paid, 10.5 + 22.44)
        # self.assertEqual(order1.amount_outstanding_payment, order1.amount_total - 10.5 - 22.44)
        # self.assertEqual(order2.amount_paid, 0)
        # self.assertEqual(order2.amount_outstanding_payment, order2.amount_total)
    ...
    def test_create_payment(self):
        ...
        self.assertEqual(order.amount_outstanding_payment, order.amount_total)
        ...
        payment_move_line = payment.line_ids.filtered(lambda r: r.account_type == 'asset_receivable')
        self.assertEqual(len(payment_move_line), 1)
        self.assertNotEqual(payment_move_line, invoice_move_line)
        self.assertEqual(payment_move_line.ref, invoice_move_line.name)

        self.assertEqual(payment_move_line.linked_sale_order_id, invoice_move_line.linked_sale_order_id)

        # @TODO: check sale order payments
        # self.assertEqual(order.amount_paid, 33.00)
        # self.assertEqual(order.amount_outstanding_payment, order.amount_total - 33.00)
    ...
    def _create_order_and_invoice(self):
        ...
        self.confirm_sale_order(order)
        self.deliver_sale_order(order)

        # create and confirm invoice
        invoice = self.invoicing_sale_order(order)
        self.post_invoice(invoice)

        # all sale order lines merged into one receivable move line
        invoice_move_line = invoice.line_ids.filtered(lambda r: r.account_type == 'asset_receivable')
        self.assertEqual(len(invoice_move_line), 1)
        self.assertEqual(invoice_move_line.linked_sale_order_id, order)
        for line in (invoice.line_ids - invoice_move_line):
            self.assertTrue(not line.linked_sale_order_id)
